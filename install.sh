...
domain_check(){
    read -p "请输入你的域名信息(eg:www.idleleo.com):" domain
...
#}
acme_cron_update(){
    [ ! -f ${ssl_update_file} ] && wget -P /usr/bin "https://raw.githubusercontent.com/paniy/V2Ray_ws-tls_bash_onekey/master/ssl_update.sh"
    if [[ "${ID}" == "centos" ]];then
